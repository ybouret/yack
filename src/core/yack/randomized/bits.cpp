...
namespace yack
{
    ...
    namespace randomized
    {
        ...
        double bits:: operator()(void) noexcept
        {
            return unit_<double>( next32() );
        }

        size_t bits:: leq(const size_t value) noexcept
        {
            bits &ran = *this;
            return floor( value * ran() + 0.5);
        }

        unit_t bits::in(const unit_t a, const unit_t b) noexcept
        {
            bits        &ran = *this;
        ...
        template <> int8_t bits:: symm<int8_t>() noexcept
        {
            return int8_t( to<uint8_t>() );
        }

        template <> int16_t bits:: symm<int16_t>() noexcept
        {
            return int16_t( to<uint16_t>() );
        }

        template <> int32_t bits:: symm<int32_t>() noexcept
        {
            return int32_t( to<uint32_t>() );
        }

        template <> int64_t bits:: symm<int64_t>() noexcept
        {
        ...
        void bits:: fill(void *addr, size_t size) noexcept
        {
