...
namespace yack
{
    ...
    struct hexa
    {
        static int        convert(const int ch) noexcept; //!< 0-15 or -1 on error
        static uint8_t    up16(const uint8_t)   noexcept; //!< (b>>4) &0xf
        static uint8_t    lo16(const uint8_t)   noexcept; //!< b&0xf
