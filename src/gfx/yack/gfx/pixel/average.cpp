...
namespace yack
{
    ...
    namespace graphic
    {
        ...
        T scalar_average(const T *arr, const size_t num) throw()
        {
            assert(num>0);
            U res = 0;
        ...
        template <> uint8_t pixel<uint8_t>:: average(const uint8_t *arr, const size_t num) throw()
        {
            assert(num>0);
            return scalar_average<uint8_t,size_t>(arr,num);
        }

        template <> float pixel<float>:: average(const float *arr, const size_t num) throw()
        {
            assert(num>0);
        ...
        template <> double pixel<double>:: average(const double *arr, const size_t num) throw()
        {
            assert(num>0);
        ...
        template <> rgb pixel<rgb>:: average(const rgb *arr, const size_t num) throw()
        {
            assert(num>0);
            rgb    ans;
        ...
        template <> rgba pixel<rgba>:: average(const rgba *arr, const size_t num) throw()
        {
            assert(num>0);
            rgba   ans;
