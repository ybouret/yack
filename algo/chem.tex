...
We look for $\vec{C}^\star$ such that:
\begin{equation}
	\vec{C}^\star = \vec{C} + \bm{\nu}^T \vec{\xi} \text{ and } \vec{\Gamma}(\vec{C}^\star) \approx 0
\end{equation}

\begin{equation}
	\forall i, \;\; \Gamma_i(\vec{C}^\star) = \Gamma_i \left((\vec{C}'_i + (\vec{C}^\star - \vec{C}_i') \right) \simeq \langle \vec{\Psi}_i' \vert  (\vec{C}^\star - \vec{C}_i') \rangle
\end{equation}
so that:
\begin{equation}
	\bm{\Psi}' \left( \vec{C} + \bm{\nu}^T \vec{\xi} \right) =
	\begin{bmatrix}
	\vdots\\
	\langle \vec{\Psi}_i' \vert \vec{C}_i' \rangle \\
	\vdots\\
	\end{bmatrix}
\end{equation}
and
\begin{equation}
	...
	\begin{bmatrix}
	\vdots\\
	\langle \vec{\Psi}_i' \vert \vec{C}_i' \rangle -  \langle \vec{\Psi}_i' \vert \vec{C} \rangle \\
	\vdots\\
	\end{bmatrix}
\end{equation}
